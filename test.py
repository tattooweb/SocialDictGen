...
def command_args():
    ...
    CAPTCHA_OCR.add_argument('--ocr',action = "store",help="需识别的验证码URL")
...
if __name__ == "__main__":
    ...
    # 验证码识别
    if args.ocr:
    	if re.match(r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+',args.ocr):
            OCR(args.ocr)
    	else:
            print("请输入正确的验证码URL")
    else:
...
'''
测试语句
python3 test.py --ocr http://www.isaipu.net/common/image.jsp
'''